    ...
    <>
      ...
      <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
        {blogMenu.map((item, idx) => {
          ...
          return (
            <div key={idx} >
              <Link href={item.href || '#'} className={`flex justify-between items-center px-3 py-2 rounded-lg hover:bg-gray-800 ${patname === item.href && 'bg-gray-800'}`}>
              ...
              {item.children && <div className="px-6">
                {item.children.map((item_children, idx_children) => {
                  return (
                    <Link key={idx_children} href={item_children.href || '#'} className={`flex items-center px-3 py-2 rounded-lg hover:bg-gray-800 `}>
                      <span className="text-sm font-semibold">{item_children.title}</span>
                    </Link>
                  )
                })}
              </div>
